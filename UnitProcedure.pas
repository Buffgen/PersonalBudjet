...
Begin
    ...
    Begin
        ...
        PointerAccount := PointerAccount^.Next;
    End;
    StringGridAccount.Col := 0;
    StringGridAccount.Col := 1;
End;
...
Begin
    ...
    Begin
        ...
        Begin
            ...
            DeleteJournalStringGrid(FormMain.StringGridJournal);
...
Procedure SetFocusCellStringGrid(Var StringGridAccounts: TStringGrid);
Begin
    With StringGridAccounts do
    Begin
        If RowCount <> 1 then
        Begin
            Row := 1;
...
Begin
    ...
    Begin
        ...
        Begin
            ...
            FillAccount(FormMain.StringGridAccounts);
            SetFocusCellStringGrid(FormMain.StringGridAccounts);
...
Begin
    ...
    Begin
        ...
        // Проверка на вставку символа между 0 и запятой: 01,2
        If Len >= 2 then
            If (CursorPos = 1) and (Key <> #8) and (Copy(Line, 1, 2) = '0,') then
                Key := #0;

        PosPoint := FindPosPoint(Line);

        // Проверка повторного ввода запятой
        If ((Key = ',') and (PosPoint <> 0)) then
            Key := #0;
